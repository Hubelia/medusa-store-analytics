...
export const SalesTotalShippingChart = ({orderStatuses, currencyCode, dateRange, dateRangeCompareTo, compareEnabled} : 
  ...
  const currentShipping = data.analytics.current.reduce((sum, item) => sum + parseInt(item.shipping), 0);
  const previousShipping = data.analytics.previous.length > 0 ? 
    data.analytics.previous.reduce((sum, item) => sum + parseInt(item.shipping), 0) : undefined;
  ...
  const rawChartData = {
    current: data.analytics.current.map(currentData => {
      return {
        ...
        value: amountToDisplay(parseInt(currentData.shipping), data.analytics.currencyDecimalDigits)
      };
    }),
    previous: data.analytics.previous.map(previousData => {
      return {
        date: new Date(previousData.date),
        value: amountToDisplay(parseInt(previousData.shipping), data.analytics.currencyDecimalDigits)
      };
    }),
  };
    ...
    <Grid container paddingBottom={2} spacing={3}>
      ...
      <Grid item xs={12}>
        <Grid container alignItems={'center'} spacing={2}>
          <Grid item>
            <h1>
              {amountToDisplay(currentShipping, data.analytics.currencyDecimalDigits)} {data.analytics.currencyCode.toUpperCase()}
            </h1>
          </Grid>
          ...
          <Grid item>
            <Grid container alignItems={'center'}>
              <Grid item>
                <IconComparison current={currentShipping} previous={previousShipping}/>
              </Grid>
              {previousShipping !== undefined && <Grid item>
                <PercentageComparison 
                  current={amountToDisplay(currentShipping, data.analytics.currencyDecimalDigits)} 
                  label={data.analytics.currencyCode.toUpperCase()} 
                  previous={amountToDisplay(previousShipping, data.analytics.currencyDecimalDigits)}
                />
              </Grid>}
            </Grid>
          </Grid>
